...
function ajax(url,postdata,ajaxfun,outtype){//outtype:json or text blob
...
function tableClass(parentbox,data,extdata,btnfun,headfun,pagefun,showtype){
	var T=this;
	...
	T.showbody=function(){
		for(let i=0;i<T.data.data.length;i++){
			...
			for(let key in T.data.head){
				if(T.data.head[key]=='')continue; //无效head
				domnew(tr,'td',null,T.data.data[i][key]);
			}
	...
	T.showlable=function(){//以标签方式显示
		...
		for(let i=0;i<T.data.data.length;i++){
			...
			for(let key in T.data.head){
				...
				domnew(line,'span',T.ext.cls.labelvalue,T.data.data[i][key]);
			}
...
function editClass(parentbox,data,extdata,buttons,btnfun,changefun){
	var E=this;
	if(parentbox.parentbox!=null){
	...
	E.show=function(){
		...
		E.body0=domnew(E.parentbox,'div');
		...
		E.body.setAttribute('style','position:relative;padding:10px;border-radius:5px;width:auto;margin:1.75rem auto;background-color:#fff;');
		...
		for(let key in E.data.head){
			...
			if(E.data.type[key].tag=='fulltxt'){//富文本
				...
				try{
					fulltxt=new fulltxtClass(labbody,E.data.data[key],key,function(htxt){
						...
						if(E.changefun!=null)E.changefun(fulltxt.body.dom,fulltxt.key,htxt);						
					});
					//fulltxt.box.setAttribute('class',E.extdata.cls.labelfulltxt);
				}catch(e){}
				if(fulltxt==null){
					...
					labinput.onblur=function(){
						...
						if(E.data.data[key]!=this.innerHTML){
							...
							if(E.changefun!=null)E.changefun(this,key,this.innerHTML);
						}
					};
				}
				continue;
			...
			if(E.data.type[key].tag=='select' && E.data.type[key].search){
				...
				labinput.onchange=function(){
					...
					if(E.changefun!=null)E.changefun(this,key,this.value);
				}
				continue;
			}
			...
			if(E.data.type[key].tag=='input'){
				if(E.data.type[key].type=='text' || E.data.type[key].type=='password'){
					if(E.data.data[key]==null)E.data.data[key]='';
					labinput.type=E.data.type[key].type;
		...
		let btncancle=domnew(labsave,'button',E.extdata.cls.btn,'取消');
	...
	E.reWriteCheckBox=function(key){
		...
		for(let i=0;i<E.data.type[key].select.length;i++){
			...
			ck.onclick=function(){
				...
				if(E.changefun!=null)E.changefun(this,key,E.data.data[key]);
			};
		}
	}
	...
	E.uploadfile=function(upurl,file,start,tname,upok){
		let psize=1024*1024;
		...
		xhr.addEventListener('load', function() {
			...
			if(jobj.result=='ok'){			
				tname=jobj.tname;
				if(jobj.filestate==0)
					E.uploadfile(upurl,file,start+psize,tname,upok);
				else{
					upok(file,jobj);
					return;
				}
	...
	E.inputok=function(finshfun){//finshfun 输入完成后的回调函数
		...
		for(let key in E.edit){
			if(E.edit[key].file!=null && E.data.type[key].upurl!=null){
				E.uploadfile(E.data.type[key].upurl,E.edit[key].file,0,'',function(file,jobj){
					if(file==null){alert('上传文件'+E.edit[key].file.name+'失败！');return;}
					E.edit[key].file=null;
					E.data.data[key]=jobj.data.filename;
					E.inputok(finshfun);
				});
				return;//有文件上传
			}
